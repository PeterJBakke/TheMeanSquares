...
def train_with_negative_sampling(train_iter, val_iter, net, test_iter, optimizer, criterion, num_user, num_epochs=5):
    ...
    for batch in train_iter:
        users,(items, lengths) = batch.user, batch.doc_title
        net.train()

        pos_and_neg_batch = negative_sampling(users, items, num_user)
        lengths = torch.cat((lengths, lengths), 0)
        target_pos = Variable(torch.tensor([1 for _ in range(len(users))]).to(device))
        target_neg = Variable(torch.tensor([0 for _ in range(len(users))]).to(device))

        output = net(pos_and_neg_batch, lengths).reshape(-1)
...
def train_without_negative_sampling(train_iter, val_iter, net, test_iter, optimizer, criterion, num_user, num_epochs=5):
    net.train()
    prev_epoch = 0
    train_loss = []
    train_error = []
